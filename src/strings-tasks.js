...
function isPalindrome(str) {
  const reverseStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  let max = sentenceArray[0];
  for (let i = 0; i < sentenceArray.length; i += 1) {
    if (sentenceArray[i].length > max.length) max = sentenceArray[i];
  }
  return max;
}
/**
...
function extractNameFromTemplate(value) {
  return value
    .split('Hello, ')[1]
    .slice(0, value.split('Hello, ')[1].length - 1);
}
/**
